...
public interface RideRepo extends JpaRepository<Ride,Long> {
    ...
    @Query("from com.srmtech.automobilepoolingapp.model.Ride where passenger_id=?1")
    List<Ride> getuserride(Long userId);
    ...
    @Query("from com.srmtech.automobilepoolingapp.model.Ride where passenger_id=?1 and date=?2")
    List<Ride> getPassengerRideDetails(Long userId, Date date);
